...
在 UI 中，终态目录可以是一个具体计算目录，也可以是包含多个 `task.*` 子目录的父目录；程序会自动判断单个或批量计算。表面能的初态目录可留空，吸附能、掺杂能和缺陷能需要显式选择初态目录。掺杂能通过用户输入的 dopant/host 原子序号从 POSCAR/CONTCAR 识别元素并自动读取单质库能量；缺陷能通过初态结构中被移除元素的原子序号读取对应单质库能量。
